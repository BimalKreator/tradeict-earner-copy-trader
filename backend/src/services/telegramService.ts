...
function utcYesterdayRange(): { start: Date; end: Date } {
  const now = new Date();
  const end = new Date(
...
export async function sendDailyPnLSummaries(prisma: PrismaClient): Promise<void> {
  if (!telegram) return;
  ...
  for (const row of grouped) {
    ...
    try {
      ...
      console.warn(
        `[telegram] daily summary failed for ${user.email}:`,
        err,
      );
    }
  }
...
export function initTelegramCronJobs(prisma: PrismaClient): void {
...
export function initTelegramBot(prisma: PrismaClient): void {
  const token = process.env.TELEGRAM_BOT_TOKEN?.trim();
  if (!token) {
    console.warn(
      "[telegram] TELEGRAM_BOT_TOKEN not set; bot and outbound alerts disabled",
    );
    return;
  }
  ...
  bot.start(async (ctx) => {
    try {
      ...
      if (!pendingUser) {
        await ctx.reply(
          "Invalid or expired link code. Generate a new code from TradeICT (Settings).",
        );
        return;
      }
      ...
      await ctx.reply(
        `✅ Linked to TradeICT account ${pendingUser.email}. You will receive trade alerts and daily PnL summaries here.`,
      );
    } catch (err) {
...
export function telegramLinkExpiry(): Date {
