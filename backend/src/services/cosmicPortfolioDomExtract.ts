...
export async function extractCosmicPortfolioDom(
  ...
  const waitMs =
    Number.isFinite(parsed) && parsed >= 3000 ? parsed : 60_000;
  ...
  return page.evaluate(() => {
    ...
    const tokenSpans = Array.from(
      document.querySelectorAll("span.font-text, span[class*='font-text']"),
    ).filter((s) => instrumentRe.test(text(s)));
    ...
    for (const tokSpan of tokenSpans) {
      ...
      let sideRaw = "";
      if (row.querySelector("span.text-green-500")) sideRaw = "BUY";
      else if (row.querySelector("span.text-red-500")) sideRaw = "SELL";
      ...
      if (entry === null) {
        ...
        const m = rowText.match(
          /(?:entry|avg\.?\s*entry)\s*[:\s]*([\d,.]+)/i,
        );
        if (m) entry = parseNum(m[1] ?? "");
      }
