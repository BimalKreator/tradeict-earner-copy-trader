...
async function generateSubscriptionInvoices(prisma: PrismaClient): Promise<void> {
  const now = new Date();
...
async function pauseSubscriptionsForOverdueInvoices(
  prisma: PrismaClient,
): Promise<void> {
