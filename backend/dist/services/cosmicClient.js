...
export async function probeCosmicOpenPositions(cosmicEmail, cosmicPassword, captureScreenshot) {
    const { payloads, screenshotBase64 } = await scrapeCosmicPositionsData(cosmicEmail.trim(), cosmicPassword.trim(), captureScreenshot ? { captureScreenshot: true } : undefined);
    const out = {
